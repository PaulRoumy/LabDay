...
public class PlayerShoot : NetworkBehaviour
{
   ...
   [SerializeField]
    ...
    {
        Debug.Log("ok");
         GameObject[] tourelles = GameObject.FindGameObjectsWithTag("Tourelle");
        int I=0;
        foreach (GameObject enemy in tourelles)
        {
            if (turretId == enemy.name)
            {
                Turret cible = tourelles[I].GetComponent<Turret>();
                
                cible.TakeDamage(currentweapon.damage);
            }
            I++;
        }
